...
class RLModelValidator:
    ...
    def compare_with_baseline(self, baseline_data):
        ...
        for rl_result in self.validation_results['temperature_accuracy']:
            ...
            if baseline_match:
                energy_improvement = (
                    (baseline_match['energy_consumption'] - rl_result['energy_consumption']) 
                    / baseline_match['energy_consumption'] * 100
                )
                
                time_improvement = (
                    (baseline_match['melting_time'] - rl_result['melting_time']) 
                    / baseline_match['melting_time'] * 100
                )
                ...
                comparison = {
                    ...
                    'baseline_temp_accuracy': baseline_match.get('temp_accuracy', 'N/A')
                }
                
                comparison_results.append(comparison)
                
                print(f"\nScenario: {scenario}")
                print(f"  Energy: RL={rl_result['energy_consumption']:.1f} kWh, "
                      f"Baseline={baseline_match['energy_consumption']:.1f} kWh "
                      f"(Improvement: {energy_improvement:.1f}%)")
                print(f"  Time: RL={rl_result['melting_time']:.1f} min, "
    ...
    def performance_metrics_analysis(self):
        ...
        metrics = {
            ...
            'energy_consumption_std': np.std([r['energy_consumption'] for r in results]),
            'melting_time_std': np.std([r['melting_time'] for r in results]),
            'scenarios_count': len(results)
        }
        
        print(f"Average Temperature Accuracy: {metrics['avg_temp_accuracy']:.2f}% ± {metrics['temp_accuracy_std']:.2f}%")
        print(f"Average Energy Consumption: {metrics['avg_energy_consumption']:.2f} ± {metrics['energy_consumption_std']:.2f} kWh")
        print(f"Average Melting Time: {metrics['avg_melting_time']:.2f} ± {metrics['melting_time_std']:.2f} minutes")
    ...
    def _plot_scenario_results(self, result, scenario_num):
        ...
        axes[0].axhline(y=result['target_temp'], color='r', linestyle='--', linewidth=2, label='Target Temperature')
        axes[0].set_ylabel('Temperature (°C)')
        axes[0].set_title(f'Scenario {scenario_num}: {result["scenario"]} - Temperature Profile')
        axes[0].legend()
        ...
        energy_cumulative = np.cumsum([p/60 for p in powers])  # Approximate cumulative energy
        axes[2].plot(times, energy_cumulative, 'm-', linewidth=2, label='Cumulative Energy')
        ...
        plt.tight_layout()
        plt.savefig(f'validation_results_scenario_{scenario_num}.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def generate_validation_report(self, output_file='rl_validation_report.md'):
        ...
        if 'performance_metrics' in self.validation_results:
            ...
            report.append(f"- Average Temperature Accuracy: {metrics['avg_temp_accuracy']:.2f}% ± {metrics['temp_accuracy_std']:.2f}%")
            report.append(f"- Average Energy Consumption: {metrics['avg_energy_consumption']:.2f} ± {metrics['energy_consumption_std']:.2f} kWh")
            report.append(f"- Average Melting Time: {metrics['avg_melting_time']:.2f} ± {metrics['melting_time_std']:.2f} minutes")
        ...
        if 'temperature_accuracy' in self.validation_results:
            ...
            report.append("| Scenario | Target Temp (°C) | Achieved Temp (°C) | Error (°C) | Accuracy (%) | Energy (kWh) | Time (min) |")
            report.append("|----------|------------------|-------------------|------------|--------------|--------------|------------|")
            
            for result in self.validation_results['temperature_accuracy']:
                report.append(f"| {result['scenario']} | {result['target_temp']} | {result['achieved_temp']:.1f} | {result['temp_error']:.1f} | {result['temp_accuracy']:.1f} | {result['energy_consumption']:.1f} | {result['melting_time']:.1f} |")
            report.append("")
        
        # Baseline comparison
        if 'baseline_comparison' in self.validation_results:
            report.append("## Baseline Comparison")
            report.append("| Scenario | RL Energy (kWh) | Baseline Energy (kWh) | Energy Improvement (%) | RL Time (min) | Baseline Time (min) | Time Improvement (%) |")
            report.append("|----------|-----------------|----------------------|------------------------|---------------|---------------------|----------------------|")
            
            for comp in self.validation_results['baseline_comparison']:
                report.append(f"| {comp['scenario']} | {comp['rl_energy']:.1f} | {comp['baseline_energy']:.1f} | {comp['energy_improvement']:.1f} | {comp['rl_time']:.1f} | {comp['baseline_time']:.1f} | {comp['time_improvement']:.1f} |")
            report.append("")
