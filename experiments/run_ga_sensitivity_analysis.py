...
def run_full_analysis():
    ...
    print(
        f"⏱️  Estimated time: {total_combinations * 2:.0f}-{total_combinations * 5:.0f} minutes"
    )
...
def show_results_summary(analyzer):
    ...
    for param_name, results in analyzer.results.items():
        ...
        best_idx = min(
            range(len(best_fitness_values)), key=lambda i: best_fitness_values[i]
        )
        ...
        if worst_fitness != best_result["best_fitness"] and worst_fitness != 0:
            improvement = (
                (worst_fitness - best_result["best_fitness"]) / worst_fitness * 100
            )
