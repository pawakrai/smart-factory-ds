...
class GASensitivityAnalyzer:
    ...
    def _run_ga_with_config(self, config):
        ...
        # Create problem
        problem = HGAProblem(NUM_BATCHES)
    ...
    def _extract_metrics_from_result(self, result):
        ...
        if result.history and len(result.history) > 0:
            ...
            metrics["final_fitness"] = generation_best_fitness[-1] if generation_best_fitness else float('inf')
            
            # Calculate convergence generation (when fitness improvement becomes minimal)
            convergence_generation = self._find_convergence_generation(generation_best_fitness)
            ...
            if len(generation_best_fitness) >= 10:
                ...
                diversity = np.std(late_gen_fitness) if len(late_gen_fitness) > 1 else 0.0
            ...
            if len(generation_best_fitness) > 1:
                ...
                if initial_fitness != 0 and not np.isinf(initial_fitness):
                    fitness_improvement = (initial_fitness - final_fitness) / initial_fitness
                else:
                    fitness_improvement = 0.0
            ...
            if len(generation_best_fitness) >= 20:
                early_improvement = generation_best_fitness[0] - generation_best_fitness[19]
                convergence_rate = early_improvement / 20 if generation_best_fitness[0] != 0 else 0.0
    ...
    def _find_convergence_generation(self, fitness_history, improvement_threshold=0.001):
        """
        ...
        for i in range(window_size, len(fitness_history)):
            ...
            if window_start_fitness != 0 and not np.isinf(window_start_fitness):
                improvement_rate = (window_start_fitness - current_fitness) / window_start_fitness
    ...
    def _generate_sensitivity_summary(self):
        ...
        for param_name, results in self.results.items():
            ...
            fitness_sensitivity = np.std(best_fitness_values) / np.mean(best_fitness_values) if np.mean(best_fitness_values) != 0 else 0
            convergence_sensitivity = np.std(convergence_generations) / np.mean(convergence_generations) if np.mean(convergence_generations) != 0 else 0
            time_sensitivity = np.std(execution_times) / np.mean(execution_times) if np.mean(execution_times) != 0 else 0
            ...
            print(f"  Best Convergence: {convergence_generations[best_idx]} generations")
            print()
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            if self.results:
                ...
                f.write("| Parameter | Best Value | Best Fitness | Convergence Gen | Sensitivity |\n")
                f.write("|-----------|------------|--------------|-----------------|-------------|\n")
                ...
                for param_name, results in self.results.items():
                    ...
                    f.write(f"| {param_name} | {best_value} | {best_fitness:.2f} | {best_convergence} | {fitness_sensitivity:.3f} |\n")
                
                f.write("\n## Detailed Analysis\n\n")
                for param_name, results in self.results.items():
                    f.write(f"### {param_name}\n\n")
                    f.write("| Value | Best Fitness | Convergence Gen | Exec Time (s) | Diversity |\n")
                    f.write("|-------|--------------|-----------------|---------------|----------|\n")
                    
                    for result in results:
                        f.write(f"| {result['value']} | {result['best_fitness']:.2f} | {result['convergence_generation']} | {result['execution_time']:.2f} | {result['diversity_maintenance']:.3f} |\n")
                    f.write("\n")
