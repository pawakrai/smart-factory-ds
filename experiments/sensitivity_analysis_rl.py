...
class RLSensitivityAnalyzer:
    ...
    def _plot_parameter_sensitivity(self, param_name, results):
        ...
        axes[0, 1].plot(values, convergence_episodes, 's-', linewidth=2, markersize=8, color='orange')
        ...
        # Training Stability
        axes[0, 2].plot(values, training_stability, '^-', linewidth=2, markersize=8, color='green')
        ...
        # Final Accuracy
        axes[1, 0].plot(values, final_accuracy, 'd-', linewidth=2, markersize=8, color='red')
        ...
        # Training Time
        axes[1, 1].plot(values, training_time, 'v-', linewidth=2, markersize=8, color='purple')
        ...
        plt.tight_layout()
        plt.savefig(f'{self.output_dir}/sensitivity_{param_name}.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _create_radar_chart(self, ax, param_name, results):
        ...
        metrics_names = ['Final Reward', 'Convergence Speed', 'Stability', 'Accuracy', 'Training Time']
        
        # Extract and normalize values
        values = [r['value'] for r in results]
        final_rewards = [r['final_reward'] for r in results]
        convergence_episodes = [1000 / r['convergence_episodes'] * 100 for r in results]  # Invert (faster is better)
        training_stability = [(1 - r['training_stability']) * 100 for r in results]  # Invert (lower variance is better)
        final_accuracy = [r['final_accuracy'] * 100 for r in results]
        training_time = [100 / r['training_time'] * 60 for r in results]  # Invert (faster is better)
        ...
        ax.set_ylim(0, 100)
        ax.set_title(f'Best {param_name} Performance Profile\n(Value: {values[best_idx]})')
        ax.grid(True)
    ...
    def _generate_sensitivity_summary(self):
        ...
        for param_name, results in self.results.items():
            ...
            reward_sensitivity = (max(final_rewards) - min(final_rewards)) / np.mean(final_rewards)
            convergence_sensitivity = (max(convergence_episodes) - min(convergence_episodes)) / np.mean(convergence_episodes)
            stability_sensitivity = (max(training_stability) - min(training_stability)) / np.mean(training_stability)
    ...
    def _plot_sensitivity_summary(self, summary_data):
        ...
        ax1.bar(x - width, reward_sensitivities, width, label='Reward Sensitivity', alpha=0.8)
        ax1.bar(x, convergence_sensitivities, width, label='Convergence Sensitivity', alpha=0.8)
        ax1.bar(x + width, stability_sensitivities, width, label='Stability Sensitivity', alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig(f'{self.output_dir}/sensitivity_summary.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def generate_report(self):
        ...
        report.append("This report presents the results of parameter sensitivity analysis for the DQN model.")
        report.append("The analysis evaluates how changes in hyperparameters affect training performance.")
        report.append("")
        ...
        for param_name, results in self.results.items():
            ...
            report.append(f"- **Best Value**: {results[best_idx]['value']} (Reward: {results[best_idx]['final_reward']:.2f})")
            report.append(f"- **Worst Value**: {results[worst_idx]['value']} (Reward: {results[worst_idx]['final_reward']:.2f})")
            report.append(f"- **Performance Range**: {max(final_rewards) - min(final_rewards):.2f}")
            report.append("")
            
            # Detailed table
            report.append("### Detailed Results")
            report.append("| Value | Final Reward | Convergence Episodes | Training Stability | Final Accuracy | Training Time |")
            report.append("|-------|--------------|---------------------|-------------------|----------------|---------------|")
            
            for r in results:
                report.append(f"| {r['value']} | {r['final_reward']:.2f} | {r['convergence_episodes']:.0f} | {r['training_stability']:.3f} | {r['final_accuracy']:.3f} | {r['training_time']:.1f} |")
            
            report.append("")
        
        # Recommendations
        report.append("## Recommendations")
        report.append("Based on the sensitivity analysis, the following parameter values are recommended:")
        report.append("")
        ...
        report.append("")
        report.append("These recommendations are based on maximizing final reward performance.")
        report.append("Consider other factors such as training time and stability when making final decisions.")
        
        # Save report
        with open(f'{self.output_dir}/sensitivity_analysis_report.md', 'w', encoding='utf-8') as f:
            f.write('\n'.join(report))
