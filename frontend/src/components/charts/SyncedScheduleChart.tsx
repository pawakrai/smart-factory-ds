...
export default function SyncedScheduleChart({ plan, batches, scheduleData }: Props) {
  ...
  // ── Current time ───────────────────────────────────────────────────────
  const nowMin = (Date.now() - shiftStart.getTime()) / 60_000;
        ...
        {showNow && (
          <>
            <line
              ...
              stroke={C_NOW} strokeWidth={1.5} opacity={0.55} strokeDasharray="4 3"
            />
        ...
        {solarVisible && (
          <>
            <line x1={solarX1} y1={PAD.top} x2={solarX1} y2={MH_Y + MH_H}
              stroke="#CA8A04" strokeWidth={1} strokeDasharray="4 2" opacity={0.6} />
