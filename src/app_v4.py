...
def _greedy_simulate_mh_state_to_minute(
    ...
    for t_sim in range(last_simulated_minute + 1, target_minute):
        ...
        is_break_time_sim = False
        for break_start, break_end in BREAK_TIMES_MINUTES:
            if break_start <= minute_of_day_sim < break_end:
                is_break_time_sim = True
                break
        ...
        for furnace_id_mh in ["A", "B"]:
            ...
            if is_break_time_sim or new_downtime_remaining_min[furnace_id_mh] > 0:
...
def greedy_assignment(batch_order, num_batches=NUM_BATCHES):
    ...
    for (
        ...
        for potential_if_start_slot in range(TOTAL_SLOTS - T_MELT + 1):
            ...
            # Check if M&H is in break time at the exact pour minute
            is_break_at_pour_minute = False
            for break_start, break_end in BREAK_TIMES_MINUTES:
                if break_start <= potential_if_melt_finish_minute < break_end:
                    is_break_at_pour_minute = True
                    break
            if is_break_at_pour_minute:
                continue  # Cannot pour during break
            ...
            # If both M&H A and B are in downtime that extends to or past potential_if_melt_finish_minute,
            # this check is implicitly handled by capacity check (as they won't be consuming).
            # But more explicitly, a pour action itself induces downtime.
            # Here, we are checking capacity *at* the melt finish minute.
