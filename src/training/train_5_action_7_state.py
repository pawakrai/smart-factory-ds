...
from src.environment.aluminum_melting_env_6 import AluminumMeltingEnvironment
...
def train_agent(
    env, agent, episodes=5000, save_model_every=500, save_path="./checkpoints_env_8"
):
    ...
    for episode in range(episodes):
        ...
        # แสดงสรุปผลของ episode (ปรับ log ให้แสดงข้อมูลที่เกี่ยวข้องกับ spark penalty ด้วย)
        ...
        logger.info(
            f"  Spark Penalty Accumulated: {env.state.get('spark_penalty',0):.2f}"
        )
