...
from src.visualization.plot_training2 import (
...
def train_agent(
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            # Env state: [temp, weight, time, power, status, energy_kwh, scrap_added, wall_temp]
        ...
        # ปรับการแสดงผล Action ให้เป็นตัวเลขกำลังไฟ (kW)
        action_percentages = {
            f"{env.action_space[i]:.0f} kW": (action_counts[i] / total_actions) * 100
        ...
        while not done:
            if DEBUG_EVAL_Q_VALUES and ep == 0 and step_i < int(DEBUG_EVAL_Q_STEPS):
                try:
                    ...
                    # ปรับให้แสดงผลเป็นกำลังไฟ (kW) แทน String
                    logger.info(
                        f"Eval step={step_i} Q={np.round(q, 3)} | argmax={top} ({env.action_space.get(top, 0):.0f} kW)"
                    )
                except Exception:
                    pass
...
if __name__ == "__main__":
    ...
    logger.info("Environment: AluminumMeltingEnvironment (version 11)")
    ...
    final_model_path = "models/dqn_final_model_env11.pth"
