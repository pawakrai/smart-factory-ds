...
class AluminumMeltingEnvironment:
    ...
    def add_scrap(self):
        ...
        if (self.scrap_addition_start <= current_time <= self.scrap_addition_end 
            ...
            if (len(self.scrap_additions) == 0 or time_since_last_scrap >= 5 * 60):
                ...
                if self.current_mass + scrap_weight <= self.max_capacity:
                    ...
                    self.scrap_additions.append({
                        'time': current_time,
                        'weight': scrap_weight,
                        'temperature_drop': scrap_weight * 0.5  # scrap ทำให้อุณหภูมิลดลง
                    })
                    ...
                    temp_drop = (scrap_weight / self.current_mass) * (self.state["temperature"] - self.ambient_temp) * 0.3
    ...
    def calculate_temperature_change(self):
        ...
        if len(self.scrap_additions) > 0:
            ...
            recent_scrap = [s for s in self.scrap_additions 
                          if self.state["time"] - s['time'] < 5 * 60]  # 5 นาทีหลังเพิ่ม scrap
            if recent_scrap:
    ...
    def step(self, action):
        ...
        self.state["energy_consumption"] += float((self.state["power"] * self.dt) / 3600)
