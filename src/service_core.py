...
"""
Core scheduling service wrapper around the HGA implementation in app_v5.py.

This module exposes a single entrypoint `run_hga_schedule` that:
- accepts a small config dictionary (or pydantic model on the API side)
- optionally overrides some global parameters in app_v5
- runs the NSGA-II optimization once
- returns the best schedule and cost components in a JSON‑serialisable dict
"""
...
def _apply_config_overrides(cfg: Dict[str, Any]) -> None:
    ...
    This keeps things flexible for the first version of the service without
    doing a big refactor of the HGA code. Only keys that exist in cfg
    will be overridden.
    """
    ...
    if isinstance(init_level, dict):
        app_v5.MH_INITIAL_LEVEL_KG.update(
            {k: float(v) for k, v in init_level.items() if k in app_v5.MH_INITIAL_LEVEL_KG}
        )
...
def run_hga_schedule(config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ----------
    config:
        Optional dictionary containing high‑level overrides, e.g.:
        {
          "num_batches": 10,
          "if": {"use_furnace_a": true, "use_furnace_b": true},
          "mh": {
    ...
    # Choose the first non‑dominated solution as the "recommended" one.
            ...
            schedule_list.append(
                {
                    ...
                    "start_min": int(start_slot * app_v5.SLOT_DURATION + app_v5.SHIFT_START),
                    "end_min": int(end_slot * app_v5.SLOT_DURATION + app_v5.SHIFT_START),
                }
            )
    ...
    response: Dict[str, Any] = {
        ...
        "summary": {
            ...
            "mh_low_level_penalty": float(cost_details.get("mh_low_level_penalty", 0.0)),
        ...
        "schedule": sorted(schedule_list, key=lambda x: (x["start_min"], x["batch_id"])),
    }
