...
def test_improved_environment():
    ...
    while not done and step_count < max_steps:
        ...
        if step_count % 10 == 0:
            ...
            print(
                f"Minute {current_minute}: Temp={state[0]:.1f}°C, "
    ...
    if final_scrap_info["scrap_additions"]:
        ...
        for i, addition in enumerate(final_scrap_info["scrap_additions"]):
            print(
                f"  Round {i+1}: {addition['weight']:.1f} kg at minute {addition['time']/60:.1f}"
            )
...
def create_simulation_plots(
    ...
    fig.suptitle(
        "Improved Aluminum Melting Environment Simulation Results", fontsize=16
    )
    ...
    axes[0, 2].axhline(
        y=350, color="r", linestyle="--", alpha=0.7, label="Initial Al ingot"
    )
    axes[0, 2].axhline(
        y=500, color="orange", linestyle="--", alpha=0.7, label="Max capacity"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "improved_aluminum_melting_simulation.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
